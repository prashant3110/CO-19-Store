...
def cart(request, userid):
    dict = {'Tomato': 'tomato.jpg', 'Potato': 'potato.jpg', 'Cabbage': 'Cabbage.jpg', 'cauliflower': 'cauliflower.jpg',
            'Garlic': 'Garlic.jpeg', 'Onion': 'onion.jpg', 'Coriander': 'Coriander.jpg', 'Lemon': 'lemon.jpg',
            'Apple': 'apples.jpg', 'Orange': 'orange.jpg', 'Banana': 'banana.jpg', 'Pomegranate': 'Pomegranate.gif'}
...
def update(request):
    if request.method == "POST":
        u = user.objects.filter(id=request.POST['id']).update(name=request.POST['name'],
...
def addtocart(request, userid):
    if request.method == 'POST':
        itm = Item(userid_id=userid, name=request.POST['item_name'], price=request.POST['option'],
